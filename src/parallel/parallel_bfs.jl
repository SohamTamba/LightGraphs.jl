...
function push_batch!(q::ThreadQueue{T,N}, to_queue::Vector{T}) where T where N
    # TODO: check that head > tail
    old_tail = atomic_add!(q.tail, convert(N, size(to_queue)[1]))
    
    old_tail -= 1
    for i in 1:size(to_queue)[1]   
        q.data[old_tail+i] = to_queue[i]
    end
end
...
function bfskernel(
    ...
    part_size = cld(size(level)[1], nthreads())  #Partition level according to threads
    @threads for part_id in 0:(nthreads()-1)
        left = part_id*part_size+1
        right = min(size(level)[1], left + part_size-1)
        local_queue = Vector{T}()

        for it_part in left:right
            src = level[it_part]
            ...
            for vertex in vertexneighbors
                if !seen[vertex]
                    ...
                    if parent == 0
                        push!(local_queue, vertex) #Push local onto queue if newly found
                        seen[vertex] = true
                    end
                end
            end
        end
        if size(local_queue)[1] > 0
            push_batch!(next, local_queue)
        end
    end
end
...
function bfs_tree!(
    ...
    while !isempty(next)
        ...
        bfskernel(alg, next, g, parents, level, seen) # Find new frontier
    end
    return parents
end
