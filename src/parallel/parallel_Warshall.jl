...
function floyd_warshall_shortest_paths(
    ...
    for pivot in vertices(g)
        pivot_dists = dists[pivot, :]
        pivot_parents = parents[pivot, :]
        @threads for u in vertices(g)
            if u != pivot  #continue not supported for multi-threaded loop
                for v in vertices(g)
                    if dists[u, pivot] == typemax(T) || pivot_dists[v] == typemax(T)
                        ans = typemax(T)
                    else
                        ans = dists[u, pivot] + pivot_dists[v]
                    end
                    if dists[u, v] > ans
                        dists[u, v] = ans
                        parents[u, v] = pivot_parents[v]
                    end
                end
            end
        end
